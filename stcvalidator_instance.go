...
// You will need to assert the error if it's not nil eg. err.(validator.ValidationErrors) to access the array of errors.
// validate Array, Slice and maps fields which may contain more than one error
func (v *Validate) VarWithKey(key string, field any, tag string) error {
...
// You will need to assert the error if it's not nil eg. err.(validator.ValidationErrors) to access the array of errors.
// validate Array, Slice and maps fields which may contain more than one error
func (v *Validate) VarWithKeyCtx(ctx context.Context, key string, field any, tag string) (err error) {
	if len(tag) == 0 || tag == skipValidationTag {
		return nil
	}
...
func getErrMsgFromMap(field, tag string, mapErrMsg MapErrMsg) string {
	for f, tags := range mapErrMsg {
		if f == field {
			for t, msg := range tags {
				if t == tag {
					return msg
				}
			}
...
func (v *Validate) StructWithMapErr(s any, mapErrMsg MapErrMsg) error {
	...
	if msg := getErrMsgFromMap(valErr.Field(), valErr.ActualTag(), mapErrMsg); msg != "" {
